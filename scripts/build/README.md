...
- `_build_ecu.sh` — Inner build script (called by the above); quiet by default (shows only the *Build Finished* summary line on success, full output on failure). Pass `--no-quiet` for full verbose output (used by CI/Docker scripts)
