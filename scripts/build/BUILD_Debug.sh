...
bash "${SCRIPT_DIR}/_build_ecu.sh" TARGET_ECUA Debug --no-clean --quiet
cp -pr "${WORKSPACE}/firmware/${PROJECT_NAME}/Debug/${PROJECT_NAME}.hex" "${OUTPUT_FOLDER}/ECUA.hex"
bash "${SCRIPT_DIR}/_build_ecu.sh" TARGET_ECUB Debug --skip-import --no-clean --quiet
cp -pr "${WORKSPACE}/firmware/${PROJECT_NAME}/Debug/${PROJECT_NAME}.hex" "${OUTPUT_FOLDER}/ECUB.hex"
bash "${SCRIPT_DIR}/_build_ecu.sh" TARGET_ECUC Debug --skip-import --no-clean --quiet
cp -pr "${WORKSPACE}/firmware/${PROJECT_NAME}/Debug/${PROJECT_NAME}.hex" "${OUTPUT_FOLDER}/ECUC.hex"
bash "${SCRIPT_DIR}/_build_ecu.sh" TARGET_ECUD Debug --skip-import --no-clean --quiet
