...
void RAMN_Encode_Control_Shift(uint8_t value, uint8_t* payload) {
    uint8_t packed = (uint8_t)PACK_SIGNAL(value, CONTROL_SHIFT_MASK, CONTROL_SHIFT_OFFSET);
    payload[0] = packed;
}
...
void RAMN_Encode_Joystick(uint8_t value, uint8_t* payload) {
    uint16_t packed;
    RAMN_memcpy((uint8_t*)&packed, payload, sizeof(packed));
    packed &= ~(JOYSTICK_MASK << JOYSTICK_OFFSET);
    packed |= (uint16_t)PACK_SIGNAL(value, JOYSTICK_MASK, JOYSTICK_OFFSET);
    RAMN_memcpy(payload, (uint8_t*)&packed, sizeof(packed));
}
